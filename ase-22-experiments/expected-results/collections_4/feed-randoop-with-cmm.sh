...
srcFolder="commons-collections4-4.1-src/src/main/java"
jarFile="commons-collections4-4.1.jar"

echo "Batch generation of weaved RANDOOP tests"
...
while read classFile <&3; do   
    ...
    java -classpath ../libs/$randoopJar:$jarFile randoop.main.Main gentests --randomseed=$1 --testclass=$targetClass --junit-output-dir=./$specsDir-$specsDirName/$randpTestDir/ --use-jdk-specifications=false --cmm-temporal-specs=$targetClass"_cmmRandoop.json"
    
done 3<classlist.txt