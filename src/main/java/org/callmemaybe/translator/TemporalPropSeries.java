...
public class TemporalPropSeries {
   ...
   * @throws IllegalArgumentException if the number of temporalRelations is not exactly 1 less than
   *     the number of propositions, unless both are empty
   */
  TemporalPropSeries(
  ...
  public List<String> getAllPredicates() {
    return propositions.stream().map(Proposition::getPredicate).collect(Collectors.toList());
  }
